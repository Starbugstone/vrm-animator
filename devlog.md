...
Latest implementation note: backend diagnostics now use Symfony Monolog, and the shared OpenAI-compatible provider layer tolerates more reasoning-era completion shapes such as `output_text` while logging sanitized payload details when a provider returns reasoning without visible assistant text. On the frontend, conversational motion handoff is now smoother: reply-time body gestures use a small priority queue, user-message-driven thinking cues outrank in-flight reply gestures, and lower-priority motions wait for a safe handoff window instead of cutting off the current action immediately.
